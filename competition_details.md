...
|------------------|--------|-------------------|-----------------------------|
| 精簡模型         | 23     | 0.801             | 未提交（由於Private資料上，大家都只丟了融合模型的）         |
| 融合模型（Best） | 5000+（全組員 stacking） | 0.833             | Top 3%           |
...
### 🧩 挑戰一：變數過多、關鍵過少
- 原始特徵超過萬個，但真正與飆股高度相關的變數極少
- 傳統特徵選擇方法無法有效濾出穩定且具泛化力的特徵組

### 🔀 挑戰二：Stacking 效果受限於模型相似性
- Stacking 模型需依賴多個「互補性高」的基礎模型
- 由於組員間密集討論與策略交流，所產生的模型結構與變數使用趨於一致，導致融合效果有限
...
- 效能接近全特徵融合模型（F1 ≈ 0.83），但模型結構更簡潔、訓練更有效率
...
本專案最終在公開資料上達成 **0.8366** 的 Macro F1，在私有測試資料上亦有穩定表現，Macro F1 為 **0.8262**，總成績穩居 Private Leaderboard 的前 **3%**。
