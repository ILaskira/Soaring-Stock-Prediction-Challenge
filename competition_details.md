...
| 挑戰 | 解法 | 驗證 |
|------|------|------|
| 🧩 特徵數量龐大、關鍵訊息稀少 | - 使用 **XGBoost Importance** 與 **Permutation Importance** 進行雙方法篩選  <br> - 取交集 (23 個) 建立 **Model 1**，差集建 **Model 2/3** 擴展多樣性 | - **Model 1** 測試 F1 ≈ 0.84  <br> - 差集模型 F1 ≈ 0.65，但保留結構差異 |
| 🔀 Stacking 效果受限於模型同質性 | - 交集/差集模型設計使三模型 **兩兩互斥** <br> - 提升模型間差異性，避免結構重疊 | - Stacking 三模型後 Public F1 ≈ 0.82 <br> - 接近最佳融合模型 (F1 ≈ 0.83)，結構更簡潔 |
| 📌 如何確保關鍵變數不被遺漏 | - 建立 **Model 4**，使用剩餘 ~9800 特徵 <br> - 驗證被排除的特徵是否仍有貢獻 | - **Model 4** 測試 F1 ≈ 0.05 <br> - 證明大部分關鍵變數已被完整涵蓋 |
